...
def simulate_ffp():
    ...
    u0 = 0.001
    tE = 1.0
    start_jd = 2460331.500000
    end_jd = 2460360.500000
    ra = 270.0
    dec = -27.0
    piEN = 0.01
    piEE = 0.01
    ...
    horizons_table = jplhorizons_utils.parse_JPL_Horizons_table(horizons_file_path='wfirst_observer_table.txt', 
                                                                table_type='OBSERVER')
                                                                
    spacecraft_positions = jplhorizons_utils.extract_spacecraft_positions(horizons_table,t0)
        
    wfirst_lc = simulate_lightcurve(start_jd, end_jd, 0.25/24.0, wfirst_aperture)
    ...
    lsst_pylima_params = extract_event_parameters(ffp_lsst, fit_lsst, parameters, blend_ratio)
    wfirst_pylima_params = extract_event_parameters(ffp_wfirst, fit_wfirst, parameters, blend_ratio)
...
def extract_event_parameters(lens, fit, event_params, blend_ratio):
        
    source_flux = lens.telescopes[0].lightcurve_magnitude[:,1].min()
...
def simulate_lightcurve(start_jd, end_jd, cadence, aperture, day_gaps=False):
    ...
    if day_gaps:
        ...
        idx = np.where(dec_days >= 9.0/24.0)[0]
        jdx = np.where(dec_days <= 11.5/24.0)[0]
...
def plot_fitted_lightcurves(lc1, lc2, event1, event2, label1, label2, file_path,
    ...
    if plot_models:
        ...
        plt.plot(ts1,model1,linestyle='dashed',
                     color='#4c1377')
         
        plt.plot(ts2,model2,linestyle='solid',
                     color='black')
